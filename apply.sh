...
original_author="jdmonty"
original_name="py_flask_api"
original_urlname="py-flask-api"
original_description="Awesome py_flask_api created by jdmonty" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/py_flask_api "${name}"
...
then
    ...
    echo "then run 'py_flask_api' or 'python -m py_flask_api'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'py_flask_api' or 'python -m py_flask_api' or 'poetry run py_flask_api'"
fi
